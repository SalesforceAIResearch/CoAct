...
def compare_table(result: str, expected: str = None, **options) -> Tuple[float, str]:
    ...
    try:
        ...
        return 0., f"Failed to load result file: {result}"
    ...
    for r in options["rules"]:
        if r["type"] == "sheet_name":
            ...
            if not metric:
                failure_reason = f"Sheet names mismatch: {worksheetr_names} != {worksheete_names}"
            #  }}} Compare Sheet Names # 
            ...
            if sheet1 is None:
                return 0., f"Failed to load sheet: {r['sheet_idx0']}"
            ...
            if not metric:
                failure_reason = f"Sheet data mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Sheet Data by Internal Value # 
            ...
            if sheet1 is None:
                return 0., f"Failed to load sheet for print comparison: {r['sheet_idx0']}"
            ...
            if not metric:
                failure_reason = f"Sheet print mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Sheet Data by Printed Value # 
            ...
            if not metric:
                failure_reason = f"Sheet fuzzy match mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Fuzzy Match for Ranges # 
            ...
            if not metric:
                failure_reason = f"Sparkline mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Sparklines # 
            ...
            if not metric:
                failure_reason = f"Chart mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Charts # 
            ...
            if not metric:
                failure_reason = f"Style mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Style (Also Conditional Formatiing) # 
            ...
            if sheet1 is None:
                return 0., f"Failed to load sheet for freeze comparison: {r['sheet_idx0']}"
            ...
            if not metric:
                failure_reason = f"Freeze mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Freezing # 
            ...
            if sheet is None:
                return 0., f"Failed to load sheet for zoom check: {r['sheet_idx']}"
            ...
            logger.debug("Assertion: %s.zoom(%.1f) %s %.1f - %s", r["sheet_idx"], zoom_scale, r["method"], r["ref"],
                         metric)
            if not metric:
                failure_reason = f"Zoom mismatch for {r['sheet_idx']}"
            #  }}} Check Zooming # 
            ...
            sheet: Worksheet = _load_sheet(*parse_idx(r["sheet_idx"], xlworkbookr, xlworkbooke))
            if sheet is None:
                return 0., f"Failed to load sheet for data validation: {r['sheet_idx']}"
            ...
            metric: bool = total_metric
            if not metric:
                failure_reason = f"Data validation mismatch for {r['sheet_idx']}"
            #  }}} Check Data Validation # 
            ...
            if not metric:
                failure_reason = f"Row properties mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Check Row Properties # 
            ...
            if not metric:
                failure_reason = f"Column properties mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Check Row Properties # 
            ...
            if not metric:
                failure_reason = f"Filter mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Filters # 
            ...
            if not metric:
                failure_reason = f"Pivot table mismatch for {r['sheet_idx0']} and {r['sheet_idx1']}"
            #  }}} Compare Pivot Tables # 
            ...
            sheet: Worksheet = _load_sheet(*parse_idx(r["sheet_idx"], xlworkbookr, xlworkbooke))
            if sheet is None:
                return 0., f"Failed to load sheet for cell check: {r['sheet_idx']}"
            ...
            if not metric:
                failure_reason = f"Cell mismatch for {r['sheet_idx']} at {r['coordinate']}"
            #  }}} Check Cell Properties # 
...
def compare_csv(result: str, expected: str, **options) -> Tuple[float, str]:
    if result is None:
        return 0., "Result CSV file is None"

    try:
        with open(result) as f:
            result_lines: Iterable[str] = f.read().splitlines()
    except Exception as e:
        return 0., f"Failed to read result CSV file: {str(e)}"
    
    try:
        with open(expected) as f:
            expected_lines: Iterable[str] = f.read().splitlines()
    except Exception as e:
        return 0., f"Failed to read expected CSV file: {str(e)}"
    ...
    if metric:
        return 1., "CSV files match"
    else:
        return 0., "CSV files do not match"


def compare_conference_city_in_order(actual_city_list_path, expected_city) -> Tuple[float, str]:
    expected_city_list = expected_city["expected"]
    try:
        wb = openpyxl.load_workbook(actual_city_list_path)
    except Exception as e:
        return 0., f"Failed to load workbook: {str(e)}"
    ...
    try:
        for i in range(len(actual_city_list)):
            if isinstance(expected_city_list[i], str):
                if expected_city_list[i] not in actual_city_list[i]:
                    ...
                    return 0., f"City mismatch at index {i}: expected '{expected_city_list[i]}' not found in '{actual_city_list[i]}'"
                ...
                if not any(possible_str in actual_city_list[i] for possible_str in expected_city_list[i]):
                    logger.debug(f"Expected city {expected_city_list[i]}; Actual city {actual_city_list[i]}")
                    print(f"Expected city {expected_city_list[i]}; Actual city {actual_city_list[i]}")
                    return 0., f"City mismatch at index {i}: none of {expected_city_list[i]} found in '{actual_city_list[i]}'"
        ...
        return 0., f"Error during comparison: {str(e)}"

    return 1., "All cities match in order"
